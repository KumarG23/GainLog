...
`health_v2_shadow_validation.py` evaluates Sleep, Recovery, Load, and Stress over the same dates, checks their missing-data/range/count invariants, compares wearable exercise evidence with GainLog-native workout dates, and emits aggregate-only receipts. Correlations are observational diagnostics rather than pass/fail health claims.

The 32-day production review for 2026-08-20 through the exclusive end date 2026-09-21 found:

- zero structural violations across all four models;
- 32 common calendar days, with Load available on all 32, Sleep and Recovery available on 31, and a mature Stress baseline on 25;
- 21 of 22 GainLog workout dates matched wearable exercise evidence; the one unmatched date was the older boundary day without retained Sleep/Recovery evidence, while the fully covered 31-day window matched all 21 logged workout dates and included two additional wearable-only active days;
- Sleep and Recovery moved together (`Spearman 0.806`, `n=31`), which is expected because Sleep is an explicit 40% Recovery component and is not independent validation;
- prior-day Load had effectively no whole-window association with next-day Recovery (`-0.006`, `n=31`), so v0.1 must not claim to predict recovery from training load;
- same-day Load and high-activation fraction were mildly positive (`0.290`, `n=25`), consistent with more physiological activation on training days;
