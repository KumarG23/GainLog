...
async function performAutoSyncUnsafe({
  requestPermissions,
  force = false,
}: {
      ...
      ...(hasHealthAccess ? [{
        ...
        run: async () => {
          await syncHealthConnect({
            ...
            repairIfRequired: requestPermissions,
            days: 2,
          });
        },
      }] : []),
...
const performAutoSync = createForceAwareTaskRunner(performAutoSyncUnsafe);
...
export function HealthConnectSyncCoordinator() {
  ...
  const syncWhenActive = useCallback(async () => {
    if (Platform.OS !== 'android') return;
    try {
      const result = await performAutoSync({ requestPermissions: true, force: true });
      ...
      // Settings keeps explicit sync and repair actions as recovery.
