...
import { clockLabel } from '../../utils/dayJourney';
...
export function DayStress({ data, onNote }: { data: HealthV2Stress; onNote: (minute: number) => void }) {
  ...
  const pick = (x: number) => {
    const minute = Math.max(0, Math.min(1439, x / width * 1440));
    const i = data.segments.findIndex((s, n) => minute >= s.startMinute && minute < (data.segments[n + 1]?.startMinute ?? 1440));
    if (i >= 0) select(i);
  };
  return <View style={j.section}><View style={[j.row, { justifyContent: 'space-between' }]}><Text style={j.heading}>Your day, in context</Text><Text style={j.note}>Stress · experimental</Text></View>
    ...
    <View style={[j.row, { justifyContent: 'space-between' }]}>{['00:00', '06:00', '12:00', '18:00', '24:00'].map(t => <Text key={t} style={j.note}>{t}</Text>)}</View>
    ...
    <Text style={j.note}>{data.scoredMinutes} eligible minutes scored · retrospective, not streaming. Touch or drag to explore.</Text>
  </View>;
}
