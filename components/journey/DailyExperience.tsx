...
export function DailyExperience() {
  ...
  const brief = buildDailyBrief({ now: journey.now, today: current, modelStale: !!feed.error, healthStale: !!health.error, workoutsStale: !!workouts.error, sessions: workouts.sessions, meals: health.nutritionEntries, check: journey.error ? emptyDay(date) : journey.check, preferences: journey.preferences, planTitle: template?.title ?? null });
  ...
  const saveIntent = async (trainingIntent: 'plan' | 'rest') => { try { await journey.saveDay(date, { trainingIntent }); setPanel(null); } catch (e) { setSaveError(e instanceof Error ? e.message : 'Not saved.'); } };
