...
export function CheckInSheet({ initial, mode = 'checkin', onClose, stressMinute }: { initial: DayCheckIn; mode?: 'checkin' | 'reflection'; onClose: () => void; stressMinute?: number }) {
  ...
  const [entryDate] = useState(initial.date), [draftRevision] = useState(initial.revision);
  const staleDraft = (journey.snapshot?.days.find(d => d.date === entryDate)?.revision ?? 0) !== draftRevision;
  ...
  return <Sheet visible title={mode === 'reflection' ? 'Close the day' : 'How do you feel?'} onClose={() => { if (!journey.busy) onClose(); }}>
    <View style={{ gap: 18 }}>
      ...
      {mode === 'checkin' && <>
        ...
        <Text style={j.heading}>Today’s intention</Text><View style={j.row}><Choice disabled={journey.busy} label="Review my plan" selected={draft.trainingIntent === 'plan'} onPress={() => setDraft(v => ({ ...v, trainingIntent: v.trainingIntent === 'plan' ? null : 'plan' }))} /><Choice disabled={journey.busy} label="Rest day" selected={draft.trainingIntent === 'rest'} onPress={() => setDraft(v => ({ ...v, trainingIntent: v.trainingIntent === 'rest' ? null : 'rest' }))} /></View>
      ...
      {staleDraft && <Text style={j.body} accessibilityLiveRegion="polite">The saved entry changed. Close and reopen this editor to review the latest version; your draft has not overwritten it.</Text>}
