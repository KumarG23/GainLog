...
export function SleepPlanSheet({ onClose }: { onClose: () => void }) {
  ...
  const save = async (clear = false) => {
    try { await journey.savePreferences(clear ? { wakeTime: null, sleepMinutes: null } : { wakeTime, sleepMinutes: minutes, windDownMinutes: wind }); onClose(); } catch (e) { setError(e instanceof Error ? e.message : 'Not saved.'); }
  };
  return <Sheet visible title="Plan tonight" onClose={() => { if (!journey.busy) onClose(); }}><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ gap: 16 }}>
    ...
    <Text style={j.heading}>Wake time · 24-hour clock</Text><TextInput style={j.input} accessibilityLabel="Wake time, HH colon MM" placeholder="06:30" placeholderTextColor="#9BA8B6" value={wakeTime} onChangeText={setWake} maxLength={5} keyboardType="numbers-and-punctuation" />
    <Text style={j.heading}>Time in bed · hours</Text><TextInput style={j.input} accessibilityLabel="Chosen time in bed in hours" placeholder="Choose 5–11 hours" placeholderTextColor="#9BA8B6" value={hours} onChangeText={setHours} keyboardType="decimal-pad" maxLength={4} />
    <Text style={j.heading}>Wind-down lead time · minutes</Text><TextInput style={j.input} accessibilityLabel="Wind-down minutes" value={windDown} onChangeText={setWindDown} keyboardType="number-pad" maxLength={3} />
    ...
    {error && <Text style={j.body} accessibilityLiveRegion="polite">{error}</Text>}
    <Button title={journey.busy ? 'Saving…' : 'Save my sleep plan'} onPress={() => void save()} disabled={!valid || !journey.snapshot || journey.busy || !!journey.error} />
    {journey.preferences.wakeTime && <Button secondary title="Remove this sleep plan" disabled={journey.busy || !!journey.error} onPress={() => void save(true)} />}
